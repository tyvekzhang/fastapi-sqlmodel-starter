...
def test_create_role(login, client, endpoint, test_data, expected_status_code, expected_code):
    access_token, user_id = login
    headers = {"Authorization": f"Bearer {access_token}"}
    response = client.post(f"{configs.api_version}/{endpoint}", json=test_data, headers=headers)
    assert response.status_code == expected_status_code
    assert response.json()["code"] == expected_code
...
def test_list_ordered_role(login, client, endpoint, expected_status_code, expected_code):
    access_token, user_id = login
    headers = {"Authorization": f"Bearer {access_token}"}
...
def test_remove_role_by_ids(login, client, endpoint, test_data, expected_status_code, expected_code):
    access_token, user_id = login
    headers = {
