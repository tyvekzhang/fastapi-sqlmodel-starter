...
def test_import_user_error(login, client, endpoint, expected_status_code, expected_code):
    access_token, user_id = login
    headers = {
        "Authorization": f"Bearer {access_token}",
        "content_type": "multipart/form-data",
...
def test_list_user(login, client, endpoint, test_data, expected_status_code, expected_code):
    access_token, user_id = login
    headers = {"Authorization": f"Bearer {access_token}"}
    response = client.post(
        f"{server_config.api_version}/user/{endpoint}",
